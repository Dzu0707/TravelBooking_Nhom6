...
const AdminBookings = () => {
  ...
  const filteredBookings = bookings.filter((b: any) => {
    ...
    const customerEmail = (b.contactEmail || '').toLowerCase();
    const specialRequest = (b.specialRequest || '').toLowerCase();
    ...
    return (
      ...
      (
        ...
        specialRequest.includes(searchStr) ||
    ...
    <div className="mx-auto max-w-7xl space-y-6 px-4 pb-10 pt-6 font-sans">
      <Card className="rounded-3xl border-slate-800 bg-slate-900 p-6 shadow-2xl">
        <div className="flex flex-col items-start justify-between gap-4 lg:flex-row lg:items-center">
          <div>
            <Title className="flex items-center gap-2 text-slate-100 font-black uppercase italic tracking-tight">
        ...
        <Table>
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow>
                <TableCell colSpan={6} className="p-20 text-center">
                  <Text className="text-slate-600 uppercase font-black italic tracking-widest">
                  ...
                  <TableRow key={b.id} className="border-b border-slate-800/50 transition-colors hover:bg-slate-800/40">
                    <TableCell className="p-5">
                      <Flex justifyContent="start" className="gap-3">
                        ...
                        <div>
                          <Text className="leading-tight tracking-tight text-slate-100 font-black uppercase italic text-[11px]">
                            {b.tourName || 'N/A'}
                          </Text>
                      ...
                      <div className="space-y-2">
                        ...
                        <Text className="flex items-start gap-2 text-[10px] italic text-amber-400">
                          ...
                          <span className="line-clamp-2">
                            {b.specialRequest || 'Không có yêu cầu đặc biệt'}
                          </span>
                        </Text>
                      </div>
                    </TableCell>
                    ...
                    <TableCell className="text-center">
                      <div className="group flex items-center justify-center gap-2">
                        <Badge
                          color="pink"
                          icon={Fingerprint}
                          className="bg-pink-500/5 px-3 py-1.5 text-[10px] font-black uppercase font-mono ring-1 ring-pink-500/30"
                        >
                          {displayCode}
                        </Badge>
                        <button
                          onClick={() => handleCopy(displayCode)}
                          className="text-slate-500 opacity-0 transition-opacity group-hover:opacity-100 hover:text-white"
                        >
                          <Copy size={14} />
                        </button>
                      </div>
                    ...
                    <TableCell className="text-center">
                      <div className="flex flex-col items-center gap-1.5">
                        ...
                        {b.status === 'Pending' && (
                          <span className="flex items-center gap-1 text-[8px] font-black uppercase animate-pulse text-rose-500">
                      ...
                      <Flex justifyContent="end" className="gap-2">
                        {actionLoading === b.id ? (
                          ...
                          <>
                            ...
                            <button className="rounded-xl border border-transparent p-2.5 text-slate-600 transition-colors hover:border-rose-500/20 hover:bg-rose-600/10 hover:text-rose-500">
                              <XCircle size={18} />
                            </button>
                          </>
                        )}
