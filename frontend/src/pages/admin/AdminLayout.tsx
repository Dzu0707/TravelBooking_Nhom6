    ...
    <div className="flex h-screen bg-slate-950 text-slate-400 font-sans antialiased overflow-hidden selection:bg-blue-500/30">
      ...
      <aside className="hidden lg:flex w-60 bg-slate-900 border-r border-slate-800 flex-col transition-all">
        ...
        <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto custom-scrollbar">
          <p className="px-3 text-[10px] font-bold uppercase tracking-[0.15em] text-slate-600 mb-2">Hệ thống</p>
          {menuItems.map((item) => {
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-slate-950">
        ...
        {/* Content Area - Loại bỏ animation slide để máy yếu load nhanh hơn */}
      ...
      <style>{`
        ...
        * { transition-duration: 150ms !important; } /* Giảm thời gian transition để cảm giác nhanh hơn */
      `}</style>
    </div>
  );
};
