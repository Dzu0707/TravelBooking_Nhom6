...
import { 
  ...
  CreditCard, Ticket, Star, Menu, ExternalLink, Home, Newspaper // Đã có Newspaper
} from 'lucide-react';
...
const AdminLayout = () => {
  ...
  const menuItems = [
    ...
    { title: 'Tin tức', path: '/admin/news', icon: <Newspaper size={18} /> }, // THÊM MỤC NÀY
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-slate-950">
        <header className="h-14 border-b border-slate-800 flex items-center justify-between px-6 bg-slate-900 z-30">
          <div className="flex items-center gap-3">
            <button className="lg:hidden p-1 text-slate-500"><Menu size={20} /></button>
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => navigate('/')} className="p-2 text-slate-500 hover:text-emerald-400 sm:hidden" title="Về trang chủ"><Home size={18} /></button>
            <button className="p-2 text-slate-500 hover:text-slate-200 relative"><Bell size={16} /><div className="absolute top-2 right-2 w-1.5 h-1.5 bg-rose-600 rounded-full" /></button>
          </div>
        </header>
