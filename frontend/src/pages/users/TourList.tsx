...
const TourList = () => {
  ...
  if (loading) return (
    <div className="h-[70vh] flex items-center justify-center bg-[#f8fbff]">
      <div className="w-9 h-9 border-4 border-blue-600 border-t-transparent rounded-full animate-spin"></div>
    </div>
  );

  return (
    <div className="w-full py-6 sm:py-10 bg-[#f8fbff] min-h-screen font-sans text-slate-900">
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 mb-8 sm:mb-10">
        <div className="flex flex-col gap-5 sm:gap-6 mb-6 sm:mb-8">
          <div className="border-l-4 border-blue-600 pl-4 sm:pl-6">
            <h1 className="text-2xl sm:text-3xl font-black uppercase italic tracking-tight text-slate-900">
              Khám Phá <span className="text-blue-600 font-normal">Hành Trình</span>
            </h1>
            <p className="text-slate-400 text-[10px] font-bold uppercase tracking-[0.2em] mt-1">
              Travel with TravelGo
            </p>
          </div>
        ...
        {/* Filter bar gọn mobile */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 bg-white p-2.5 rounded-2xl shadow-sm border border-blue-100">
          <div className="relative sm:col-span-1">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2.5 bg-slate-50 rounded-xl text-xs outline-none focus:ring-2 ring-blue-200"
            />
          </div>

          <div className="relative">
            <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 text-blue-500" size={15} />
            <select
              ...
              className="w-full pl-9 pr-9 py-2.5 bg-slate-50 rounded-xl text-xs font-semibold text-slate-700 outline-none appearance-none cursor-pointer focus:ring-2 ring-blue-200"
            >
          ...
          <button
            ...
            className="bg-blue-600 text-white rounded-xl font-bold text-[11px] uppercase tracking-wide hover:bg-blue-700 transition-all min-h-[40px]"
          >
            Tìm kiếm
          </button>
        </div>
      ...
      {/* Grid mobile-first */}
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-4 sm:gap-6 max-w-[1600px] mx-auto px-4 sm:px-6">
        {currentTours.length > 0 ? currentTours.map((tour) => {
          ...
          return (
            <div
              key={tour.id}
              className="group bg-white rounded-2xl sm:rounded-[2rem] shadow-sm hover:shadow-lg transition-all duration-300 flex flex-col border border-slate-100 relative overflow-hidden"
            >
              <div className="relative h-44 sm:h-48 p-2 grid grid-cols-3 gap-2">
                <div
                  className="col-span-2 overflow-hidden rounded-xl sm:rounded-[1.2rem] relative cursor-pointer"
                ...
                <div className="col-span-1 grid grid-rows-2 gap-2">
                  <div className="rounded-lg sm:rounded-xl overflow-hidden bg-slate-50">
                    <img src={getImgUrl(imgs[1]?.imageUrl || mainImg)} className="w-full h-full object-cover" alt="" />
                  </div>
                  <div className="rounded-lg sm:rounded-xl overflow-hidden relative bg-slate-50">
              ...
              <div className="p-4 sm:p-5 flex flex-col grow">
                <div className="flex justify-between items-center mb-2">
                  <span className="flex items-center text-blue-600 font-black text-[9px] uppercase tracking-wide">
                ...
                <h3
                  className="text-base sm:text-lg font-black text-slate-800 mb-2 uppercase italic line-clamp-1 cursor-pointer hover:text-blue-600 transition-colors"
                  onClick={() => navigate(`/tours/${tour.id}`)}
                >
                  {tour.name}
                </h3>

                <p className="text-slate-400 text-[11px] leading-relaxed line-clamp-2 mb-3 sm:mb-4 min-h-[34px]">
                  {tour.description || 'Khám phá vẻ đẹp thiên nhiên cùng hành trình tuyệt vời tại TravelGo...'}
                </p>

                <div className="flex justify-around items-center py-2.5 border-t border-slate-100 mb-3 sm:mb-4 mt-auto bg-slate-50 rounded-xl">
                  <div className="flex items-center gap-1.5">
                    <Calendar size={12} className="text-blue-500" />
                    <span className="text-[9px] font-black uppercase text-slate-400">Hàng ngày</span>
                  </div>
                  <div className="w-[1px] h-3 bg-slate-200"></div>
                  <div className="flex items-center gap-1.5">
                    <Users size={12} className="text-emerald-500" />
                    <span className="text-[9px] font-black uppercase text-slate-400">Còn chỗ</span>
                  </div>
                </div>
                ...
                <div className="mt-auto flex items-end justify-between">
                  <div className="flex flex-col">
                    ...
                    <div className="flex items-baseline gap-0.5">
                      <span className="text-lg sm:text-xl font-black text-blue-600 italic">
                  ...
                  <button
                    onClick={() => navigate(`/tours/${tour.id}`)}
                    className="bg-slate-900 text-white w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center hover:bg-blue-600 transition-all"
                  >
                    <ArrowRight size={16} />
                  </button>
                </div>
                ...
                {activeReviewId === tour.id && (
                  <div className="absolute inset-0 bg-white/98 backdrop-blur-xl z-50 p-4 sm:p-6 flex flex-col">
                    ...
                    <textarea
                      ...
                      className="w-full grow bg-slate-50 border border-slate-100 rounded-xl p-3 sm:p-4 text-[12px] outline-none mb-4 resize-none"
                    />
                    <button
                      onClick={() => handleSendReview(tour.id)}
                      className="w-full bg-blue-600 text-white py-3 rounded-xl font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-2 hover:bg-blue-700 transition-all"
                    >
          ...
          <div className="col-span-full py-16 text-center flex flex-col items-center">
            <Filter size={42} className="text-slate-200 mb-4" />
      ...
      {/* Pagination */}
      {totalPages > 1 && (
        <div className="mt-12 sm:mt-16 flex justify-center items-center gap-2 sm:gap-4 px-4">
          <button
            disabled={currentPage === 1}
            onClick={() => setCurrentPage((p) => p - 1)}
            className="p-2.5 sm:p-3 rounded-xl bg-white border border-slate-200 text-slate-400 disabled:opacity-20 hover:bg-blue-600 hover:text-white transition-all"
          >
            <ChevronLeft size={17} />
          </button>
          ...
          <div className="flex gap-2 flex-wrap justify-center">
            {[...Array(totalPages)].map((_, i) => (
              <button
                ...
                className={`w-9 h-9 sm:w-10 sm:h-10 rounded-xl font-black text-[10px] transition-all ${
                  currentPage === i + 1
