...
const TourList = () => {
  ...
  const handleSendReview = async (tourId: number) => {
    ...
    try {
      ...
      fetchData(); // Load lại để cập nhật số lượng review
    } catch (e) { 
          ...
          return (
            <div key={tour.id} className="group bg-white rounded-[2.5rem] shadow-sm hover:shadow-2xl transition-all duration-500 flex flex-col h-full border border-slate-100 relative overflow-hidden">
              ...
              <div className="p-6 flex flex-col flex-grow">
                ...
                <h3 className="text-lg font-black text-slate-800 mb-3 uppercase italic tracking-tighter line-clamp-1 group-hover:text-indigo-600 transition-colors" onClick={() => navigate(`/tours/${tour.id}`)}>
                  {tour.name}
                </h3>

                <p className="text-slate-500 text-[11px] italic leading-relaxed line-clamp-2 h-[32px] mb-5">
                ...
                {activeReviewId === tour.id && (
                  <div className="absolute inset-0 bg-white/98 backdrop-blur-md z-50 p-8 flex flex-col rounded-[2.5rem] animate-in fade-in zoom-in duration-200">
                    ...
                    <textarea value={comment} onChange={(e) => setComment(e.target.value)} placeholder="Viết cảm nhận..." className="w-full flex-grow bg-slate-50 rounded-2xl p-5 text-xs italic outline-none mb-6 resize-none shadow-inner" />
