...
const TourList = () => {
  ...
  // --- HÀM XỬ LÝ ẢNH THÔNG MINH (FIX LỖI ADMIN/USER KHÁC NHAU) ---
  const getImgUrl = (url: any) => {
    if (!url || typeof url !== 'string') {
        return "https://images.unsplash.com/photo-1501785888041-af3ef285b470?q=80&w=500";
    }

    // Trường hợp 1: Dữ liệu bị lỗi lặp 2 lần URL (như trong SQL của bạn)
    if (url.includes(`${API_BASE_URL}/${API_BASE_URL}`)) {
        return url.replace(`${API_BASE_URL}/${API_BASE_URL}`, API_BASE_URL);
    }

    // Trường hợp 2: Đã có http rồi thì trả về luôn (nhưng phải check lặp lần nữa cho chắc)
    if (url.startsWith('http')) {
        return url;
    }

    // Trường hợp 3: Đường dẫn tương đối từ Backend (/uploads/...)
  ...
  return (
    <div className="w-full py-10 bg-[#f8faff] min-h-screen font-sans text-slate-900">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
            ...
            <div className="flex items-center gap-2 overflow-x-auto no-scrollbar pb-2">
                <button 
                    ...
                    className={`px-5 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${!selectedCategoryId ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-100' : 'bg-white text-slate-400 border border-slate-100'}`}
                >
                    Tất cả
                </button>
                {categories.map((cat) => (
                    <button 
                        ...
                        className={`whitespace-nowrap px-5 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${selectedCategoryId === cat.id ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-100' : 'bg-white text-slate-400 border border-slate-100'}`}
                    >
                        {cat.name}
                    </button>
                ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3 bg-white p-3 rounded-[2rem] shadow-sm border border-slate-50">
          <div className="relative md:col-span-1">
            ...
            <input 
              ...
              className="w-full pl-10 pr-4 py-3 bg-slate-50 rounded-xl text-xs outline-none focus:ring-1 ring-indigo-200"
            />
          </div>
          ...
          <div className="relative">
            ...
            <select 
              ...
              className="w-full pl-10 pr-4 py-3 bg-slate-50 rounded-xl text-xs font-bold text-slate-700 outline-none appearance-none cursor-pointer"
            >
          ...
          <div className="relative">
             ...
             <input 
              ...
              className="w-full pl-10 pr-10 py-3 bg-slate-50 rounded-xl text-xs outline-none font-bold text-slate-500"
            />
          </div>
          
          <button className="bg-slate-900 text-white rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-indigo-600 transition-all">Tìm kiếm ngay</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 max-w-[1600px] mx-auto px-6">
        {currentTours.length > 0 ? currentTours.map((tour) => {
          ...
          // Fix logic lấy ảnh: ưu tiên lấy ảnh đầu tiên trong danh sách ảnh tour
          ...
          return (
            <div key={tour.id} className="group bg-white rounded-[2.5rem] shadow-sm hover:shadow-xl transition-all duration-500 flex flex-col h-[460px] border border-slate-100 relative overflow-hidden">
              ...
              <div className="p-5 flex flex-col grow">
                ...
                <h3 className="text-lg font-black text-slate-800 mb-2 uppercase italic line-clamp-1 cursor-pointer hover:text-indigo-600 transition-colors" onClick={() => navigate(`/tours/${tour.id}`)}>
                    {tour.name}
                </h3>
                ...
                <div className="flex justify-around items-center py-3 border-t border-slate-50 mb-4 mt-auto bg-slate-50/50 rounded-xl">
                    <div className="flex items-center gap-1.5">
                        <Calendar size={12} className="text-indigo-500"/><span className="text-[8px] font-black uppercase text-slate-400">Hàng ngày</span>
                    </div>
                    <div className="w-[1px] h-3 bg-slate-200"></div>
                    <div className="flex items-center gap-1.5">
                        <Users size={12} className="text-emerald-500"/><span className="text-[8px] font-black uppercase text-slate-400">Còn chỗ</span>
                    </div>
                </div>
                ...
                <div className="mt-auto flex items-end justify-between">
                    ...
                    <button onClick={() => navigate(`/tours/${tour.id}`)} className="bg-slate-900 text-white w-10 h-10 rounded-xl flex items-center justify-center hover:bg-indigo-600 transition-all shadow-lg">
                        <ArrowRight size={18} />
                    </button>
                </div>
                ...
                {activeReviewId === tour.id && (
                  <div className="absolute inset-0 bg-white/98 backdrop-blur-xl z-50 p-6 flex flex-col animate-in fade-in zoom-in duration-300">
                    <div className="flex justify-between items-center mb-4">
                        ...
                        <button onClick={() => setActiveReviewId(null)} className="text-slate-400 hover:text-rose-500"><X size={18} /></button>
                    </div>
                    <div className="flex justify-center gap-2 mb-4">
                      {[1, 2, 3, 4, 5].map((s) => (
                        <Star key={s} onClick={() => setRating(s)} size={20} className={`cursor-pointer ${s <= rating ? "text-amber-400" : "text-slate-100"}`} fill={s <= rating ? "currentColor" : "none"} />
                      ))}
                    </div>
                    <textarea value={comment} onChange={(e) => setComment(e.target.value)} placeholder="Trải nghiệm của bạn..." className="w-full grow bg-slate-50 border border-slate-100 rounded-xl p-4 text-[11px] outline-none mb-4 resize-none shadow-inner" />
                    <button onClick={() => handleSendReview(tour.id)} className="w-full bg-indigo-600 text-white py-3 rounded-xl font-black text-[9px] uppercase tracking-widest flex items-center justify-center gap-2 hover:bg-indigo-700 transition-all">
        ...
        <div className="mt-16 flex justify-center items-center gap-4">
          ...
          <div className="flex gap-2">
            {[...Array(totalPages)].map((_, i) => (
              <button key={i + 1} onClick={() => setCurrentPage(i + 1)} className={`w-10 h-10 rounded-xl font-black text-[10px] transition-all ${currentPage === i + 1 ? 'bg-indigo-600 text-white' : 'bg-white text-slate-400'}`}>{i + 1}</button>
            ))}
          </div>
          <button disabled={currentPage === totalPages} onClick={() => setCurrentPage(p => p + 1)} className="p-3 rounded-xl bg-white border border-slate-100 text-slate-400 disabled:opacity-20 hover:bg-indigo-600 hover:text-white transition-all"><ChevronRight size={18} /></button>
        </div>
      )}
    </div>
  );
